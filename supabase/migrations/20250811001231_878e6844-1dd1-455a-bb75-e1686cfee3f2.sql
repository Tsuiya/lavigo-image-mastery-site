...
-- Create enums
create type if not exists public.app_role as enum ('MEMBER','BLACK','ADMIN');
create type if not exists public.benefit_category as enum ('Sessão Extra','Diagnóstico','Desconto Parceiro','Evento VIP');
create type if not exists public.content_type as enum ('Guia','Modelo','Tutorial','Checklist');
create type if not exists public.campaign_objective as enum ('Tráfego WhatsApp','Leads','Promo Sazonal');
create type if not exists public.redemption_status as enum ('solicitado','aprovado','concluído','expirado');
create type if not exists public.rsvp_status as enum ('confirmado','lista_espera','cancelado');
...
create trigger if not exists trg_redemptions_code
before insert on public.redemptions
for each row execute function public.generate_redemption_code();

-- Policies
create policy if not exists "companies_owner_select" on public.companies
for select to authenticated using (owner_user_id = auth.uid() or public.is_admin(auth.uid()));
create policy if not exists "companies_owner_modify" on public.companies
...
create policy if not exists "users_extra_self_select" on public.users_extra
for select to authenticated using (user_id = auth.uid() or public.is_admin(auth.uid()));
create policy if not exists "users_extra_self_modify" on public.users_extra
...
create policy if not exists "benefits_select_non_black" on public.benefits
for select to authenticated using (not is_black_only);
create policy if not exists "benefits_select_black_only" on public.benefits
for select to authenticated using (is_black_only and public.is_black(auth.uid()));
create policy if not exists "benefits_admin_modify" on public.benefits
for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));

create policy if not exists "contents_select_non_black" on public.contents
for select to authenticated using (not is_black_only);
create policy if not exists "contents_select_black_only" on public.contents
for select to authenticated using (is_black_only and public.is_black(auth.uid()));
create policy if not exists "contents_admin_modify" on public.contents
for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));

create policy if not exists "campaigns_select_non_black" on public.campaigns
for select to authenticated using (not is_black_only);
create policy if not exists "campaigns_select_black_only" on public.campaigns
for select to authenticated using (is_black_only and public.is_black(auth.uid()));
create policy if not exists "campaigns_admin_modify" on public.campaigns
for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));

create policy if not exists "assets_select_all" on public.assets
for select to authenticated using (true);
create policy if not exists "assets_admin_modify" on public.assets
for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));

create policy if not exists "events_select_all" on public.events
for select to authenticated using (true);
create policy if not exists "events_admin_modify" on public.events
for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));

create policy if not exists "rsvps_self_select" on public.rsvps
for select to authenticated using (user_id = auth.uid() or public.is_admin(auth.uid()));
create policy if not exists "rsvps_self_insert" on public.rsvps
for insert to authenticated with check (user_id = auth.uid() or public.is_admin(auth.uid()));
create policy if not exists "rsvps_self_update" on public.rsvps
for update to authenticated using (user_id = auth.uid() or public.is_admin(auth.uid()));

create policy if not exists "redemptions_self_select" on public.redemptions
for select to authenticated using (user_id = auth.uid() or public.is_admin(auth.uid()));
create policy if not exists "redemptions_self_insert" on public.redemptions
for insert to authenticated with check (user_id = auth.uid() or public.is_admin(auth.uid()));
create policy if not exists "redemptions_self_update" on public.redemptions
for update to authenticated using (user_id = auth.uid() or public.is_admin(auth.uid()));

create policy if not exists "news_select_all" on public.news
for select to authenticated using (true);
create policy if not exists "news_admin_modify" on public.news
for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));

create policy if not exists "partners_select_all" on public.partners
for select to authenticated using (true);
create policy if not exists "partners_admin_modify" on public.partners
for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));

create policy if not exists "contracts_self_select" on public.contracts
for select to authenticated using (user_id = auth.uid() or public.is_admin(auth.uid()));
create policy if not exists "contracts_admin_modify" on public.contracts
for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));
